...
NetworkAbstraction.prototype.get = function(where, query, callback) {
    sendHttpReq("GET", where, null, query, callback);
}

NetworkAbstraction.prototype.postRaw = function(where, what, query, callback) {
    var opts = {};
    if(what != undefined) opts.body = what;
    opts.uri = url.parse("https://" + hostname + "/" + where);
    opts.method = "POST";
    ...
    specialRequest(opts, function(error, clientResp, resp) { handleResponse(error, clientResp, resp, callback) });
}
...
function handleResponse(error, clientResp, resp, callback) {
    ...
    if(error) {
        ...
        if (jsonResp.errors != undefined && jsonResp.errors.length > 0) {
